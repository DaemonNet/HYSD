...
def runReiter_simulatedObs(outfile, rulesfile, runs=100):
    ...
    for i in range (0,runs):
        rules_parsed = parseSD(readSDfromFile(rulesfile))
        num_rules=len(rules_parsed)
        s = SymptomGenerator()
        values_dict = s.getFakeSymptoms(rules_parsed, returnStringKeys=False)
        s.writeQuantumObservations(values_dict, rulesfile, outfile=os.path.basename(rulesfile)+".qobs")
